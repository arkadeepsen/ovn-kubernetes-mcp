...
if [[ "${MCP_MODE}" == "offline" ]]; then
    ...
    # Infer kind cluster name from kubeconfig current context (kind uses context name "kind-<cluster-name>")
    CURRENT_CTX=$(kubectl config current-context --kubeconfig="${KUBECONFIG}" 2>/dev/null || true)
    if [[ -n "${CURRENT_CTX}" && "${CURRENT_CTX}" == kind-* ]]; then
    ...
    if ! kubectl --kubeconfig="${KUBECONFIG}" wait --for=condition=Available deployment -A -l "${MCP_LABEL}" --timeout=60s; then
    ...
    NAMESPACE=$(kubectl --kubeconfig="${KUBECONFIG}" get deployment -A -l "${MCP_LABEL}" -o jsonpath='{.items[0].metadata.namespace}' 2>/dev/null || true)
    if [[ -z "${NAMESPACE:-}" ]]; then
        echo "Could not find deployment with label ${MCP_LABEL}; is deploy-k8s applied?"
        exit 1
    fi
    SVC=$(kubectl --kubeconfig="${KUBECONFIG}" get svc -n "${NAMESPACE}" -l "${MCP_LABEL}" -o jsonpath='{.items[0].metadata.name}' 2>/dev/null || true)
    SVC_PORT=$(kubectl --kubeconfig="${KUBECONFIG}" get svc -n "${NAMESPACE}" -l "${MCP_LABEL}" -o jsonpath='{.items[0].spec.ports[0].port}' 2>/dev/null || true)
    ...
    if ! kubectl --kubeconfig="$KUBECONFIG" wait --for=condition=Ready pod -n "$NAMESPACE" -l app.kubernetes.io/name=ovn-kubernetes-mcp --timeout=120s; then
        echo "MCP server pod did not become Ready within 120s"
        kubectl --kubeconfig="$KUBECONFIG" get pods -n "$NAMESPACE" -l app.kubernetes.io/name=ovn-kubernetes-mcp 2>/dev/null || true
        exit 1
    fi
    ...
    kubectl --kubeconfig="$KUBECONFIG" port-forward -n "$NAMESPACE" "svc/${SVC}" "${MCP_LOCAL_PORT}:${SVC_PORT}" &
    PF_PID=$!
