...
// pkgDisplayName returns a human-readable category label from the package name.
// Each hyphen-separated part is title-cased and joined with spaces.
// E.g. "must-gather" -> "Must Gather", "network-tools" -> "Network Tools".
func pkgDisplayName(pkg string) string {
...
func main() {
	...
	for _, pkgName := range liveOrder {
		...
		liveTools[pkgDisplayName(pkgName)] = tools
	}
	for _, pkgName := range offlineOrder {
		...
		offlineTools[pkgDisplayName(pkgName)] = tools
	}
	...
	fmt.Fprintln(&out, "|---------------|------|-------------|")
	for _, pkgName := range liveOrder {
		cat := pkgDisplayName(pkgName)
		for i, t := range liveTools[cat] {
			prefix := "| **" + cat + "** |"
			if i > 0 {
				prefix = "| |"
			}
	...
	for _, pkgName := range offlineOrder {
		cat := pkgDisplayName(pkgName)
		for i, t := range offlineTools[cat] {
			prefix := "| **" + cat + "** |"
			if i > 0 {
				prefix = "| |"
			}
...
// the first line or first sentence for the README table.
